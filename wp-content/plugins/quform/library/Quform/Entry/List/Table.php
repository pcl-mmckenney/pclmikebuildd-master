...
class Quform_Entry_List_Table extends WP_List_Table
{
    ...
    protected function _column_icon($item, $classes)
    {
        ...
        if ($item['unread'] == '1') {
            ...
            $output .= '<i class="fa fa-envelope-open-o"></i>';
        }
    ...
    public function getLabelSearchHtml()
    {
        ...
        $output = '<div id="qfb-entry-label-search-trigger" class="qfb-entry-label-search-trigger"><i class="fa fa-tags"></i></div>';
        $output .= '<div id="qfb-entry-label-search" class="qfb-entry-label-search">';

        foreach ($this->labels as $label) {
            $output .= sprintf(
                '<div class="qfb-entry-label%s" style="background-color:%s" data-id="%d"><span class="qfb-entry-label-name">%s</span><i class="fa fa-check"></i></div>',
