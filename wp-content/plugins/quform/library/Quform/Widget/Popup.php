    ...
    public function form($instance)
    {
        ...
        if ( ! count($forms)) {
            if (current_user_can('quform_add_forms')) {
                printf('<p>' . esc_html__('No forms yet, %sclick here to create one%s.', 'quform') . '</p>', '<a href="' . esc_url(admin_url('admin.php?page=quform.forms&sp=add')) . '">', '</a>');
            } else {
        ...
        <p>
            ...
            <br>
            <small><?php printf(esc_html__('JSON encoded options to pass to the popup script, %sexamples%s.', 'quform'), '<a href="http://support.themecatcher.net/quform-wordpress-v2/guides/customization/popup-script-options" target="_blank">', '</a>'); ?></small>
        </p>
        <?php
    }
