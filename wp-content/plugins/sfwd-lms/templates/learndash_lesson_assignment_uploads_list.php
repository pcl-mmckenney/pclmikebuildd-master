				...
				<?php foreach( $assignments as $assignment ) { ?>
					<tr>
						<td class="ld-assignment-delete-col"><?php 
							...
							<a href="<?php echo add_query_arg('learndash_delete_attachment', $assignment->ID) ?>" title="<?php _e('Delete this uploaded Assignment', 'learndash'); ?>"><?php _e('X', 'learndash' ); ?></a><?php 
								} 
							}
						?></td>
						<td class="ld-assignment-filename-col">
							<a href='<?php echo esc_attr( get_post_meta( $assignment->ID, 'file_link', true ) ); ?>' target="_blank"><?php echo __( 'Download', 'learndash' ) . ' ' . get_post_meta( $assignment->ID, 'file_name', true ); ?></a><br />
							<span class="learndash_uploaded_assignment_points"><?php echo learndash_assignment_points_awarded( $assignment->ID ); ?></span>
						</td>
						<td class="ld-assignment-comments-col"><a href='<?php echo esc_attr( get_permalink( $assignment->ID ) ); ?>'><?php _e( 'Comments', 'learndash' ); ?></a></td>
					</tr>
				<?php } ?>
			</table>
		</div>
