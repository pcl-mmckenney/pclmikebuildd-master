...
function add_essays_data_columns( $cols ) {
	...
	foreach( $cols as $col_key => $col_label ) {
		if ($col_key == 'title') {
			$cols_new[$col_key] = __( 'Essay Question Title', 'learndash' );
		} else if ($col_key == 'author') {
			$cols_new[$col_key] 			= __( 'Submitted By', 'learndash' );
			
			// We add all out own columns after 'author' 
			$cols_new['approval_status'] 	= __('Status', 'learndash' );
			$cols_new['approval_points'] 	= __('Points', 'learndash' );
			$cols_new['course'] 			= 	sprintf( _x( 'Assigned %s', 'Assigned Course Label', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ) );
			$cols_new['lesson'] 			= 	sprintf( _x( 'Assigned %s', 'Assigned Lesson', 'learndash' ), LearnDash_Custom_Label::get_label( 'lesson' ) );
			$cols_new['quiz']				= 	sprintf( _x( 'Assigned %s', 'Assigned Quiz', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) );
		...
		case 'approval_points':
			...
			if ( ! empty( $quiz_id ) ) {
				...
				if ( $question instanceof WpProQuiz_Model_Question ) {
					...
					if ( $essay->post_status == 'not_graded' ) {
						?><button id="essay_approve_<?php echo $post_id ?>" class="small essay_approve_single"><?php _e('approve', 'learndash'); ?></button><?php
					}
				} else {
					echo '-';
				}
...
function learndash_essay_inline_actions( $actions, $post ) {
	if ( $post->post_type == 'sfwd-essays' ) {
		...
		if ( ! empty( $upload ) ) {
			$actions['download_essay'] = '<a href="' . esc_url( $upload ) . '" target="_blank">' . __( 'Download', 'learndash' ) . '</a>';
		} 
		/*
		if ($post->post_status == 'not_graded') {
			$approve_link = '';
			$actions['approve_essay'] = "<a href='" . $approve_link . "' target='_blank'>" . __( 'Approve', 'learndash' ) . '</a>';
		}
		*/
	}
					...
					<?php					
						...
						if ( empty( $essay_quiz_post_id ) ) {
							...
							if ( count( $essay_quiz_query->posts ) > 1 ) {
								?>
								<p>
								<strong><?php _e( 'Essay Quiz', 'learndash' ); ?>:</strong> 
								<select name="essay_quiz_post_id">
									<option value=""><?php _e('No Quiz', 'learndash' ); ?></option>
									<?php
						...
						if ( !empty( $essay_quiz_post_id ) ) {
							...
							?><p>
							<strong><?php _e( 'Essay Quiz', 'learndash' ); ?>:</strong> <?php echo get_the_title( $essay_quiz_post_id ); ?> <?php
								if (!empty( $essay_quiz_edit_link ) ) {
									?><span>(<a href="<?php echo $essay_quiz_edit_link; ?>"><?php _e( 'edit', 'learndash' ); ?></a>)</span><?php
								}
								?>
							</p>
							<?php
							...
							if ( !empty( $essay_quiz_course_id ) ) {
								...
								?>
								<p>
								<strong><?php _e( 'Essay Course', 'learndash' ); ?>:</strong> <?php echo get_the_title( $essay_quiz_course_id ); ?> <?php
									if (!empty( $course_quiz_edit_link ) ) {
										?><span>(<a href="<?php echo $course_quiz_edit_link; ?>"><?php _e( 'edit', 'learndash' ); ?></a>)</span><?php
									}
									?>
								</p>
								<?php
								...
								if ( !empty( $essay_quiz_lesson_id ) ) {
									...
									?>
									<p>
									<strong><?php _e( 'Essay Lesson', 'learndash' ); ?>:</strong> <?php echo get_the_title( $essay_quiz_lesson_id ); ?> <?php
										if (!empty( $lesson_quiz_edit_link ) ) {
											?><span>(<a href="<?php echo $lesson_quiz_edit_link; ?>"><?php _e( 'edit', 'learndash' ); ?></a>)</span><?php
										}
										?>
									</p>
									<?php
