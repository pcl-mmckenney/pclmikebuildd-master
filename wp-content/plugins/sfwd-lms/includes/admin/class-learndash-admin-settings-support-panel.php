		...
		public function admin_page() {
			...
			<div id="learndash-settings-support" class="learndash-settings" class="wrap">
				...
				<?php
					...
					foreach( $template_array as $idx => $template_file ) {
						...
						if ( strpos( $filename, '.min.') !== false) {
							...
							$template_array[$idx] = substr($filename, 0, -4);
						} else {
				...
				<?php if ( learndash_is_admin_user() ) { ?>
					...
					<p><?php _e('<span style="color:red;">Warning: This will remove ALL LearnDash data including any custom database tables.</style></span>', 'learndash' ) ?></p>
					...
					<script>
						jQuery('form#ld_data_remove_form').submit( function( event ) {
							...
							console.log('ld_data_remove_verify[%o]', ld_data_remove_verify);
