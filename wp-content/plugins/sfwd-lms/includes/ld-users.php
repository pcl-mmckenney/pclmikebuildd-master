...
function learndash_user_get_enrolled_courses_old( $user_id = 0, $bypass_transient = false ) {
	...
	if ( $enrolled_courses_ids_transient === false ) {
		...
		$sql_str = "SELECT post_id FROM ". $wpdb->postmeta ." WHERE meta_key='_sfwd-courses' AND (meta_value REGEXP ". $course_price_type ."	OR (meta_value NOT REGEXP ". $not_like ." AND meta_value REGEXP ". $is_like ."))";
...
/*
function learndash_get_admin_user_ids( $return_count = false ) {
