...
/*
function learndash_return_bytes_from_shorthand_OLD1( $val = 0 ) {
	if ( !empty( $val ) ) {
	    $val = trim( $val );
...
function learndash_check_upload( $uploadfiles = array(), $post_id = 0  ) {
	...
	if ( ( is_array( $uploadfiles ) ) && ( !empty( $post_id ) ) ) {
		...
		if ( ( isset( $post_settings['assignment_upload_limit_size'] ) ) && ( !empty( $post_settings['assignment_upload_limit_size'] ) ) ) {
			...
			$limit_file_size = ini_get('upload_max_filesize');
		}
		$limit_file_size = learndash_return_bytes_from_shorthand( $limit_file_size );
		
		if ( intval($uploadfiles['size'][0] ) > $limit_file_size ) {
			$learndash_assignment_upload_error_message .= __( 'Uploaded file size exceeds allowed limit.', 'learndash' );
			return false;
		}
		...
		if ( ( isset( $post_settings['assignment_upload_limit_extensions'] ) ) && ( !empty( $post_settings['assignment_upload_limit_extensions'] ) ) ) {
			...
			if ( !empty( $limit_file_exts ) ) {
				...
				if ( in_array( $filename_ext, $limit_file_exts ) === false ) {
					$learndash_assignment_upload_error_message .= __( 'The uploaded file type is not allowed.', 'learndash' );
					return false;
				}
			}
		}
...
/*
error_log('file<pre>'. print_r($file, true) .'</pre>');

$mime_types = wp_get_mime_types();
error_log('mime_types<pre>'. print_r($mime_types, true) .'</pre>');

$filetype = wp_check_filetype( 'test.' . $file['name'][0], $mime_types );
error_log('filetype<pre>'. print_r($filetype, true) .'</pre>');
