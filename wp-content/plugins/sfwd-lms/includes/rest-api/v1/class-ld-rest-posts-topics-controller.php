<?php
if ( ( !class_exists( 'LD_REST_Posts_Topics_Controller' ) ) && ( class_exists( 'LD_REST_Posts_Controller' ) ) ) {
	class LD_REST_Posts_Topics_Controller extends LD_REST_Posts_Controller {
		...
		public function __construct( $post_type = '' ) {
			...
			$this->rest_base = LearnDash_Settings_Section::get_section_setting('LearnDash_Settings_Section_Permalinks', 'topics' );
	    ...
	    public function register_routes() {
			...
			*/
			...
			register_rest_route( 
				$this->namespace, 
				'/' . $this->courses_rest_base . '/(?P<course_id>[\d]+)/' . $this->lessons_rest_base . '/(?P<lesson_id>[\d]+)/' . $this->rest_base . '/(?P<id>[\d]+)/mark_complete', 
				array(
					'args' => array(
						'course_id' => array(
							'description' => __( 'Unique identifier for the Course object.' ),
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'topic_mark_complete' ),
					),
				) 
			);
	    }
		
		function topic_mark_complete( $request ) {
			...
			if ( empty( $course_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', __( 'Invalid Course ID.' ), array( 'status' => 404 ) );
			}
			
			if ( empty( $lesson_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', __( 'Invalid Lesson ID.' ), array( 'status' => 404 ) );
			}
			...
			$return = learndash_process_mark_complete( $current_user_id, $topic_id );
			if ( $return === true ) {
				$data = array( 
